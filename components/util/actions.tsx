            ...
            element = (
              <Link key={index} href={action.link ? action.link : "/"}>
                <button
                  ...
                  className={`z-10 relative flex items-center px-7 py-3 font-semibold text-lg transition duration-150 ease-out  rounded-lg transform focus:shadow-outline focus:outline-none focus:ring-2 ring-offset-current ring-offset-2 whitespace-nowrap bg-gold`}
                >
                  {action.label}
                  {action.icon && (
                    <BiRightArrowAlt
          ...
          if (action.type === "link" || action.type === "linkExternal") {
            element = (
              <Link key={index} href={action.link ? action.link : "/"} passHref>
                <a
                  data-tinafield={`${parentField}.${index}`}
                  className={`group inline-flex items-center font-semibold text-lg transition duration-150 ease-out bg-gold`}
                  style={{
